...
// Define the panel class and event handler at module top-level (macro constraint).
#[cfg(target_os = "macos")]
tauri_panel! {
...
pub fn show_panel(app_handle: &AppHandle) {
    if let Some(panel) = get_or_init_panel_impl(app_handle) {
...
pub fn toggle_panel(app_handle: &AppHandle) {
    let Some(panel) = get_or_init_panel_impl(app_handle) else {
        return;
    };
